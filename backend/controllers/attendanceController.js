...
class AttendanceController {
  ...
  async markAttendance(req, res) {
    try {
      ...
      // Map records to DB schema
      const dataToInsert = finalAttendanceRecords.map(record => ({
      ...
      const result = await prisma.attendance.createMany({
        data: dataToInsert,
        skipDuplicates: true // Optional: skip duplicates if any race condition occurs
      });
      ...
      // Use input data for summary since createMany doesn't return records
      const attendanceResults = finalAttendanceRecords;
  ...
  async markAttendanceWithExceptions(req, res) {
    try {
      ...
      // Create attendance records (OPTIMIZED)
      const dataToInsert = attendanceRecords.map(record => ({
        studentId: record.studentId,
        classRoomId,
      ...
      // Fetch created records to return full response
      const attendanceResults = await prisma.attendance.findMany({
        where: {
          classRoomId,
          date: new Date(date),
          // Add subjectId filter if applicable to scope the results specifically
          ...(subjectId ? { subjectId } : {})
        },
        include: {
          student: {
