  ...
  switch (action.type) {
    ...
    case ACTION_TYPES.UPDATE_USER:
      return {
        ...
        teachers: state.teachers.map(teacher =>
          teacher.userId === action.payload.id ? { ...teacher, user: action.payload } : teacher
        ),
        students: state.students.map(student =>
          student.userId === action.payload.id ? { ...student, user: action.payload } : student
        )
      };
    ...
    case ACTION_TYPES.DELETE_USER:
      return {
        ...
        teachers: state.teachers.filter(teacher => teacher.userId !== action.payload),
        students: state.students.filter(student => student.userId !== action.payload)
      };
...
export const AdminProvider = ({ children }) => {
  ...
  const fetchTeachers = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      dispatch({ type: ACTION_TYPES.SET_TEACHERS, payload: response.teachers || [] });
      return response;
    } catch (error) {
  ...
  const fetchStudents = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      dispatch({ type: ACTION_TYPES.SET_STUDENTS, payload: response.students || [] });
      return response;
    } catch (error) {
  ...
  const updateLeaveRequestStatus = useCallback(async (leaveId, leaveData) => {
    ...
    try {
      ...
      dispatch({ type: ACTION_TYPES.UPDATE_LEAVE_REQUEST, payload: updatedRequest });
  ...
  const fetchAttendanceOverview = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      dispatch({ type: ACTION_TYPES.SET_ATTENDANCE_OVERVIEW, payload: overview });
      return overview;
    } catch (error) {
  ...
  const fetchClassAttendanceComparison = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      dispatch({ type: ACTION_TYPES.SET_CLASS_ATTENDANCE_COMPARISON, payload: comparison });
      return comparison;
    } catch (error) {
