...
import api from '../../services/api'; // ✅ Need this for data fetching (students/classrooms/stats)

// Keep all lucide-react imports
import {
  Download,
  FileText,
  Users,
    ...
    >
      <div className="flex items-center space-x-2 md:space-x-3 flex-1 min-w-0">
        ...
        <div className="text-left min-w-0 flex-1">
          ...
          <p className="text-gray-600 text-[10px] sm:text-xs md:text-sm mt-0.5 md:mt-1 hidden sm:block line-clamp-1">{description}</p>
        </div>
      </div>
...
const PDFGenerate = () => {
  ...
  const [expandedSection, setExpandedSection] = useState('student-report');
  const [isGenerating, setIsGenerating] = useState(false);
  const [activePreview, setActivePreview] = useState(null);

  // Form states for Student Progress Report
  ...
  // Form states for Exam Mark Sheet
  ...
  // Form states for Attendance Sheet
  const [attendanceSheetData, setAttendanceSheetData] = useState({
    classRoomId: '',
    date: new Date().toISOString().split('T')[0]
  });

  // Enhanced Custom PDF with table support and theme
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Fetch students, classrooms, and stats in parallel
      const [studentsRes, classroomsRes, statsRes] = await Promise.allSettled([
        api.get('/pdf/students'),
        api.get('/pdf/classrooms'),
        api.get('/pdf/stats')
      ]);
      ...
      if (classroomsRes.status === 'fulfilled') {
        ...
        console.error('Classrooms fetch error:', classroomsRes.reason);
      }
  ...
  const handleGenerateStudentReport = useCallback(async (isPreview = false) => {
    ...
    setActivePreview(isPreview ? 'student-report' : null);
    clearError();
  ...
  const addTableRow = useCallback((tableId) => {
    setCustomPDFData(prev => ({
      ...
      tables: prev.tables.map(table =>
        table.id === tableId 
          ? { ...table, rows: [...table.rows, Array(table.headers.length).fill('')] }
          : table
      )
    }));
  }, []);
    ...
    setCustomPDFData(prev => ({
      ...
      tables: prev.tables.map(table =>
        table.id === tableId 
          ? {
              ...table,
              headers: table.headers.map((header, idx) => idx === colIndex ? value : header)
            }
          : table
      )
    }));
      ...
      tables: prev.tables.map(table =>
        table.id === tableId 
          ? {
              ...
              rows: table.rows.map(row => row.filter((_, idx) => idx !== colIndex))
            }
          : table
      )
    }));
  ...
  const filteredStudents = React.useMemo(() => {
    ...
    return students.filter(student => {
      const studentName = student.user?.name || student.fullName || student.name || '';
      ...
      return (
        ...
        (student.currentEnrollment?.classRoom?.name || '').toLowerCase().includes(searchTerm)
      );
    });
  }, [students, studentSearch]);

  // Filtered classrooms for mark sheet
  const filteredClassRoomsMarkSheet = React.useMemo(() => {
    ...
    return classRooms.filter(classroom => {
      ...
      const teacherName = classroom.teacher?.user?.name || classroom.teacherName || '';
  ...
  // Filtered classrooms for attendance
          ...
          <div className="mt-1 max-h-48 overflow-y-auto border border-gray-200 rounded-lg bg-white">
            {isLoadingStudents ? (
              ...
              filteredStudents.map((student) => {
                const studentName = student.user?.name || student.fullName || student.name || 'Unknown';
                const admissionNo = student.admissionNo || student.admissionNumber || 'N/A';
                const className = student.currentEnrollment?.classRoom?.name || 'N/A';
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      handleStudentReportChange('studentId', student.id || student._id);
                      setStudentSearch(studentName);
                    }}
                    className={`w-full text-left px-3 md:px-4 py-2 md:py-3 hover:bg-amber-50 transition-colors ${studentReportData.studentId === (student.id || student._id) ? 'bg-amber-50 border-l-4 border-amber-500' : ''}`}
                  >
                ...
                <p className="text-xs text-green-700 truncate">
                  {(() => {
                    const student = students.find(s => 
                      s.id === studentReportData.studentId || s._id === studentReportData.studentId
                    );
                    return student?.user?.name || student?.fullName || student?.name || '';
                  })()}
                </p>
              </div>
              <button
        ...
        <div className="relative">
          ...
          <div className="mt-1 max-h-48 overflow-y-auto border border-gray-200 rounded-lg bg-white">
            {isLoadingClassRooms ? (
              <div className="p-4 text-center">
                <Loader2 className="h-6 w-6 animate-spin text-amber-600 mx-auto" />
                <p className="text-xs text-gray-500 mt-2">Loading classrooms...</p>
              </div>
            ) : filteredClassRoomsMarkSheet.length === 0 ? (
              <div className="p-3 md:p-4 text-center text-gray-500 text-xs">
                {classRoomSearchMarkSheet ? 'No classrooms found' : 'No classrooms available'}
              </div>
            ) : (
              filteredClassRoomsMarkSheet.map((classroom) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      handleMarkSheetChange('classRoomId', classroom.id || classroom._id);
                      setClassRoomSearchMarkSheet(className);
                    }}
                    className={`w-full text-left px-3 md:px-4 py-2 md:py-3 hover:bg-amber-50 transition-colors ${markSheetData.classRoomId === (classroom.id || classroom._id) ? 'bg-amber-50 border-l-4 border-amber-500' : ''}`}
                  >
                ...
                <p className="text-xs text-green-700 truncate">
                  {(() => {
                    const classroom = classRooms.find(c => 
                      c.id === markSheetData.classRoomId || c._id === markSheetData.classRoomId
                    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            onChange={(e) => handleMarkSheetChange('examName', e.target.value)}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleMarkSheetChange('subjectName', e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            onChange={(e) => handleMarkSheetChange('totalMarks', e.target.value)}
            placeholder="100"
            min="1"
            className="w-full px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-500"
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleMarkSheetChange('examDate', e.target.value)}
            className="w-full px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-500"
          />
        </div>
      </div>
    </div>
  ), [markSheetData, classRoomSearchMarkSheet, classRooms, filteredClassRoomsMarkSheet, handleMarkSheetChange, isLoadingClassRooms]);
    ...
    <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <div className="mt-1 max-h-48 overflow-y-auto border border-gray-200 rounded-lg bg-white">
            {isLoadingClassRooms ? (
              ...
              <div className="p-3 md:p-4 text-center text-gray-500 text-xs">
                {classRoomSearchAttendance ? 'No classrooms found' : 'No classrooms available'}
              </div>
            ) : (
              filteredClassRoomsAttendance.map((classroom) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      handleAttendanceSheetChange('classRoomId', classroom.id || classroom._id);
                      setClassRoomSearchAttendance(className);
                    }}
                    className={`w-full text-left px-3 md:px-4 py-2 md:py-3 hover:bg-amber-50 transition-colors ${attendanceSheetData.classRoomId === (classroom.id || classroom._id) ? 'bg-amber-50 border-l-4 border-amber-500' : ''}`}
                  >
                    <div className="font-medium text-gray-900 text-xs md:text-sm">
                      {className}
                    </div>
            ...
            <div className="flex items-center justify-between">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-green-700 truncate">
                  {(() => {
                    const classroom = classRooms.find(c => 
                      c.id === attendanceSheetData.classRoomId || c._id === attendanceSheetData.classRoomId
                    );
                    return classroom?.name || '';
                  })()}
                </p>
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => handleAttendanceSheetChange('date', e.target.value)}
          className="w-full px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-500"
        />
      </div>
    </div>
  ), [attendanceSheetData, classRoomSearchAttendance, classRooms, filteredClassRoomsAttendance, handleAttendanceSheetChange, isLoadingClassRooms]);

  const renderCustomPDFContent = useCallback(() => (
    // Keep the existing custom PDF content renderer as is
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleCustomPDFChange('subtitle', e.target.value)}
        ...
        {customPDFData.content.length > 0 && (
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {customPDFData.content.map((section) => (
              <div key={section.id} className="p-2 md:p-3 border-2 border-gray-200 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] md:text-xs font-medium uppercase">{section.type}</span>
                  <button
                ...
                <textarea
                  value={section.type === 'list' ? section.items?.join('\n') : section.text}
                  onChange={(e) => updateContentSection(section.id, section.type === 'list' ? 'items' : 'text', section.type === 'list' ? e.target.value.split('\n') : e.target.value)}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5 md:gap-2">
            ...
            <label className="text-xs md:text-sm font-medium text-gray-700">Tables</label>
          </div>
          <button
            onClick={addTable}
        ...
        {customPDFData.tables.map((table) => (
          <div key={table.id} className="p-3 md:p-4 border-2 border-gray-300 rounded-lg mb-3 bg-gray-50">
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div>
                <label className="text-[10px] md:text-xs text-gray-600">Header Color</label>
                <input
                  type="color"
              ...
              <div>
                <label className="text-[10px] md:text-xs text-gray-600">Header BG</label>
                <input
                  type="color"
                  value={table.headerBgColor}
                  onChange={(e) => updateTable(table.id, 'headerBgColor', e.target.value)}
                  className="w-full h-6 md:h-8 rounded border"
                />
              </div>
              <div>
                <label className="text-[10px] md:text-xs text-gray-600">Border</label>
                <input
                  type="color"
                  value={table.borderColor}
                  onChange={(e) => updateTable(table.id, 'borderColor', e.target.value)}
                  className="w-full h-6 md:h-8 rounded border"
                />
              </div>
            </div>
      ...
      <div className="flex flex-wrap gap-3 md:gap-4">
        <label className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => handleCustomPDFChange('includeDate', e.target.checked)}
        ...
        <select
          ...
          onChange={(e) => handleCustomPDFChange('orientation', e.target.value)}
  ...
  ), [customPDFData, addContentSection, removeContentSection, updateContentSection, addTable, updateTable, addTableRow, addTableColumn, updateTableCell, updateTableHeader, removeTable, removeTableRow, removeTableColumn, updateTheme, handleCustomPDFChange]);
        ...
        <div className="mb-4 md:mb-6 lg:mb-8">
          <div className="flex flex-col xs:flex-row items-start xs:items-center gap-2 sm:gap-3 mb-3">
            <div className="p-2 md:p-3 bg-gradient-to-br from-amber-400 to-amber-600 rounded-lg md:rounded-xl shadow-lg">
              <FileText className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 lg:h-9 lg:w-9 text-white" />
            </div>
          ...
          <div className="space-y-4 md:space-y-6">
            <div className="bg-white rounded-xl shadow-lg border-2 border-amber-100 p-3 md:p-4 lg:p-6">
              ...
              <div className="space-y-2 md:space-y-3">
                <div className="flex items-center justify-between p-2 md:p-3 bg-gradient-to-r from-amber-50 to-amber-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Users className="h-3 w-3 md:h-4 md:w-4 text-gray-700" />
                    <span className="text-[10px] md:text-xs font-medium">Students</span>
                  </div>
                  {isLoadingStats ? (
                    <Loader2 className="h-3 w-3 md:h-4 md:w-4 animate-spin text-amber-600" />
                  ) : (
                    <span className="text-xs md:text-sm font-bold">{stats.students}</span>
                  )}
                </div>
                <div className="flex items-center justify-between p-2 md:p-3 bg-gradient-to-r from-amber-50 to-amber-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    <UserCheck className="h-3 w-3 md:h-4 md:w-4 text-gray-700" />
                    <span className="text-[10px] md:text-xs font-medium">Teachers</span>
                  </div>
                  {isLoadingStats ? (
                    <Loader2 className="h-3 w-3 md:h-4 md:w-4 animate-spin text-amber-600" />
                  ) : (
                    <span className="text-xs md:text-sm font-bold">{stats.teachers}</span>
                  )}
                </div>
                <div className="flex items-center justify-between p-2 md:p-3 bg-gradient-to-r from-amber-50 to-amber-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Building2 className="h-3 w-3 md:h-4 md:w-4 text-gray-700" />
                    <span className="text-[10px] md:text-xs font-medium">Classes</span>
                  </div>
                  {isLoadingStats ? (
                    <Loader2 className="h-3 w-3 md:h-4 md:w-4 animate-spin text-amber-600" />
                  ) : (
                    <span className="text-xs md:text-sm font-bold">{stats.classes}</span>
                  )}
                </div>
                <div className="flex items-center justify-between p-2 md:p-3 bg-gradient-to-r from-amber-50 to-amber-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    <CalendarDays className="h-3 w-3 md:h-4 md:w-4 text-gray-700" />
                    <span className="text-[10px] md:text-xs font-medium">Reports This Month</span>
                  </div>
                  {isLoadingStats ? (
                    <Loader2 className="h-3 w-3 md:h-4 md:w-4 animate-spin text-amber-600" />
                  ) : (
                    <span className="text-xs md:text-sm font-bold">{stats.reportsThisMonth}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl border-2 border-amber-200 p-3 md:p-4 lg:p-6 shadow-lg">
              ...
              <ul className="space-y-2 text-[10px] sm:text-xs md:text-sm text-amber-800">
                <li className="flex items-start gap-2">
                  ...
                  <span>Use date filters for specific periods in progress reports</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1 text-amber-600">•</span>
                  <span>Preview reports before downloading</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1 text-amber-600">•</span>
                  <span>Mark sheets auto-calculate totals</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1 text-amber-600">•</span>
                  <span>Custom PDFs support tables & colors</span>
                </li>
              </ul>
            </div>
              ...
              <div className="space-y-2 md:space-y-3">
                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-gray-50 rounded-lg">
                  <div className="p-1.5 md:p-2 bg-amber-100 rounded-lg">
                    <Eye className="h-3 w-3 md:h-4 md:w-4 text-amber-600" />
                  </div>
                  <div>
                    <div className="text-[10px] md:text-xs font-medium">Live Preview</div>
                    <div className="text-[10px] md:text-xs text-gray-600">View before download</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-gray-50 rounded-lg">
                  <div className="p-1.5 md:p-2 bg-amber-100 rounded-lg">
                    <Download className="h-3 w-3 md:h-4 md:w-4 text-amber-600" />
                  </div>
                  <div>
                    <div className="text-[10px] md:text-xs font-medium">Instant Download</div>
                    <div className="text-[10px] md:text-xs text-gray-600">Direct PDF download</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-gray-50 rounded-lg">
                  <div className="p-1.5 md:p-2 bg-amber-100 rounded-lg">
                    <Printer className="h-3 w-3 md:h-4 md:w-4 text-amber-600" />
                  </div>
                  <div>
                    <div className="text-[10px] md:text-xs font-medium">Print Ready</div>
                    <div className="text-[10px] md:text-xs text-gray-600">Optimized for printing</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-gray-50 rounded-lg">
                  <div className="p-1.5 md:p-2 bg-amber-100 rounded-lg">
                    <Filter className="h-3 w-3 md:h-4 md:w-4 text-amber-600" />
                  </div>
                  <div>
                    <div className="text-[10px] md:text-xs font-medium">Flexible Filters</div>
                    <div className="text-[10px] md:text-xs text-gray-600">Customize data</div>
                  </div>
                </div>
              </div>
            </div>
